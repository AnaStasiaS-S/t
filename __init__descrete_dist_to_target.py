...
from sb3_contrib import TRPO, QRDQN, CrossQ, RecurrentPPO, MaskablePPO, ARS
...
class SerpControllerEnv(Node, Env):
    def __init__(self) -> None:
        ...
        # self.actions = spaces.Box(low=np.array([-1., -1.5]), high=np.array([1., 1.5]), dtype=np.float32)
        ...
        # self.action_space = spaces.Box(low=np.array([-1., -1.5]), high=np.array([1., 1.5]), shape=(2, ), dtype=np.float32)
    ...
    def run_rl_alg(self):   
        ...
        # policy_kwargs = dict(net_arch=dict(pi=[64, 16], qf=[64, 16]))
        policy_kwargs = dict(net_arch=[64, 16])
        agent = DQN("MlpPolicy", self, verbose=1,  policy_kwargs=policy_kwargs, gamma=0.90)
